<?php
$this->assign('title', $adCategory->title);
            ...
            <div class="panel-container show">
                <div class="panel-content">
                    ...
                    <div class="d-flex border-top pt-3">
                        <?php
                        if (empty($adCategory->ads)) {
                            echo $this->Form->postLink(
                                $this->Html->tag('i', '', ['class' => 'fal fa-trash']) . ' ' . __d('panel', 'Delete'),
                                $this->Url->build(['action' => 'delete', h($adCategory->id)]),
                                [
                                    'class' => 'color-danger-900 mt-2 pr-2 mr-auto',
                                    'data-title' => __d('panel', 'Are you sure you want to delete the ad category?'),
                                    'data-message' => __d('panel', 'Deletion eliminates the possibility of data recovery.')
                                ]
                            );
                        } else {
                            echo $this->Html->tag('span', __d('panel', 'To remove a group, you must remove all related ads'), ['class' => 'text-warning']);
                        }
